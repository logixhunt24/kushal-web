        ...
        >
          <ol className="flex items-center flex-nowrap whitespace-nowrap gap-1.5 sm:gap-2 text-xs sm:text-sm font-bold tracking-tight text-white">
            <li>
              <Link to="/">
                <motion.div
                  whileHover={{ color: "var(--kushal-primary)" }}
                  className="p-2.5"
                >
                  <Home size={16} />
                </motion.div>
            ...
            <AnimatePresence mode="popLayout">
              {crumbs.map((crumb, idx) => (
                <React.Fragment key={crumb.to}>
                  ...
                  <li>
                    {idx === crumbs.length - 1 ? (
                      <span className="px-3 py-1.5 rounded-lg bg-white/20 text-white font-black">
                        {crumb.name}
                      </span>
                    ) : (
                      <Link
                        to={crumb.to}
                        className="px-3 py-1.5 hover:text-yellow-400 transition"
                      >
                        {crumb.name}
                      </Link>
                    )}
