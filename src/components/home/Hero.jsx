...
const CONFIG = {
  ...
  providerUrl:
    "https://play.google.com/store/apps/details?id=com.kushalapp.partner&pcampaignid=web_share",
  ratingValue: "4.9/5",
...
function ScreenHome() {
  return (
    <div className="flex h-full flex-col bg-white px-4 pt-10 pb-3 text-left">
      ...
      <p className="mt-4 text-[10px] font-bold tracking-wider uppercase text-neutral-400">
        Categories
      </p>
      <div className="mt-1.5 grid grid-cols-3 gap-1.5">
        {CATEGORIES.map((c) => (
          <div
            key={c.label}
            className="flex flex-col items-center gap-1 rounded-xl bg-white py-2 ring-1 ring-black/5 transition-colors hover:bg-neutral-50 cursor-pointer"
          >
            <div
              className={`grid h-7 w-7 place-items-center rounded-lg ${c.tone}`}
            >
              <c.icon className="h-3.5 w-3.5" />
            </div>
            <span className="text-[8.5px] font-bold text-neutral-700">
              {c.label}
            </span>
          </div>
        ))}
      </div>
      <p className="mt-3 text-[10px] font-bold tracking-wider uppercase text-neutral-400">
        Featured Offer
      </p>
      <div className="mt-1.5 rounded-xl bg-gradient-to-br from-[#50008C] to-[#7B1FA2] p-3 text-white shadow-md shadow-purple-900/10">
        <p className="text-[8px] font-bold tracking-wider uppercase opacity-75">
          Today's standard
        </p>
        <p className="text-[11px] font-extrabold leading-tight mt-0.5">
          Deep Home Cleaning
        </p>
        <p className="mt-1 text-[9px] font-medium bg-white/20 inline-block px-1.5 py-0.5 rounded">
          Starts at ₹499 · 4.9★
        </p>
      </div>
      <div className="mt-2 flex gap-1.5">
        ...
        <div className="flex-1 rounded-xl bg-neutral-50 p-2 ring-1 ring-black/5">
          ...
          <p className="mt-1 text-[9.5px] font-bold text-neutral-800">
            Spa Wellness
          </p>
...
function ScreenCategories() {
  return (
    <div className="flex h-full flex-col bg-white px-4 pt-8 pb-3 text-left">
      ...
      <p className="mt-0.5 text-[9.5px] font-medium text-neutral-400">
        28 certified professionals near you
      </p>
      <div className="mt-2.5 flex gap-1 overflow-hidden">
        {["All", "Top ", "Nearby", "Offers"].map((t, i) => (
          <span
            key={t}
            className={`rounded-full px-2.5 py-1 text-[8.5px] font-bold tracking-wide transition-colors cursor-pointer ${i === 0 ? "bg-[#50008C] text-white" : "bg-neutral-50 text-neutral-600 ring-1 ring-neutral-200"}`}
          >
            {t}
          </span>
        ))}
      ...
      <div className="mt-3 flex flex-col gap-1.5">
        {BEAUTY_SERVICES.map((it) => (
          <div
            key={it.name}
            className="flex items-center gap-2 rounded-xl bg-white p-2 ring-1 ring-neutral-100 hover:ring-purple-200 shadow-xs transition-all duration-300"
          >
            <div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-purple-50 text-[#50008C]">
              <Sparkles className="h-3.5 w-3.5" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-[10px] font-bold text-neutral-800">
                {it.name}
              </p>
              <div className="mt-0.5 flex items-center gap-1">
                <Star className="h-2.5 w-2.5 fill-amber-400 text-amber-400" />
                <span className="text-[8.5px] font-bold text-neutral-600">
                  {it.rating}
                </span>
                <span className="text-[8.5px] text-neutral-300">|</span>
                <span className="text-[8.5px] font-bold text-[#50008C]">
                  {it.price}
                </span>
              </div>
            </div>
...
function ScreenBooking() {
  return (
    <div className="flex h-full flex-col items-center bg-white px-4 pt-9 pb-3 text-center">
      ...
      <p className="mt-4 text-[14px] font-extrabold text-neutral-800">
        Booking Confirmed
      </p>
      <p className="text-[9.5px] font-medium text-neutral-400 mt-0.5">
        Your dynamic routing assignment is ready
      </p>
      <div className="mt-4 w-full rounded-xl bg-neutral-50 p-2.5 ring-1 ring-neutral-200/50 text-left">
        ...
        <div className="mt-1 flex items-center gap-2">
          <div className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 text-[10px] font-bold text-white">
            RV
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[10px] font-bold text-neutral-800">
      ...
      <div className="mt-2 w-full rounded-xl bg-neutral-50 p-2.5 ring-1 ring-neutral-200/50 text-left">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] font-extrabold text-[#50008C]">
            22 min
          </span>
        </div>
...
function FloatingCard({
  ...
  subtitle,
  className = "",
  iconClass = "bg-[#50008C] text-white",
}) {
  return (
    <motion.div
    ...
    >
      <div
        className={`grid h-8 w-8 shrink-0 place-items-center rounded-xl shadow-xs ${iconClass}`}
      >
        <Icon className="h-4 w-4" />
      </div>
      <div className="min-w-0 text-left">
        <p className="text-[12.5px] font-extrabold leading-tight text-neutral-900 tracking-tight">
          {title}
        </p>
        {subtitle && (
          <p className="text-[9.5px] font-semibold text-neutral-500 mt-0.5">
            {subtitle}
          </p>
        )}
      </div>
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(to right, #50008C 1px, transparent 1px), linear-gradient(to bottom, #50008C 1px, transparent 1px)",
      ...
      <div className="mx-auto grid w-full max-w-[1280px] grid-cols-1 items-center gap-[50px] lg:gap-[40px] xl:gap-[80px] px-4 sm:px-6 lg:grid-cols-[1.05fr_1fr] lg:px-10">
        ...
        >
          ...
          >
            ...
            <br />
            <span className="text-[28px] sm:text-[38px] md:text-[44px] font-medium text-neutral-400 block sm:inline mr-2">
              with
            </span>
            <span
          ...
          >
            KushalApp connects you with verified professionals for home
            maintenance, beauty, repair, delivery, and digital services — all in
          ...
          >
            ...
            >
              ...
              <span className="text-left leading-tight">
                <span className="block text-[9px] font-bold opacity-75 tracking-wider uppercase">
                  Get it on
                </span>
          ...
          >
            {TRUST_PARAMS.map((b) => (
              <div key={b.text} className="flex items-center gap-2 group">
                ...
                <span className="text-[12px] font-bold text-neutral-600 group-hover:text-neutral-900 transition-colors">
                  {b.text}
                </span>
              </div>
            ))}
          ...
          >
            ...
            <div className="text-left">
              <div className="flex items-center gap-0.5">
                ...
                <span className="ml-1 text-[12.5px] font-black text-neutral-900">
                  {CONFIG.ratingValue}
                </span>
              </div>
              <p className="text-[11px] font-medium text-neutral-500 mt-0.5">
                Loved by{" "}
                <span className="font-bold text-neutral-800">10,000+</span>{" "}
                active Indian households
              </p>
            </div>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          >
            <PhoneFrame
              className="scale-[0.80] sm:scale-[0.84]"
          ...
          <FloatingCard
            icon={Users}
            title="500+ Experts"
            subtitle="Across 40+ Hubs"
            className="left-[-15px] sm:left-[-10px] top-[4%] hidden sm:flex"
            iconClass="bg-[#50008C] text-white"
          />
          <FloatingCard
            icon={Star}
            ...
            className="right-[-15px] sm:right-[-10px] top-[10%] hidden sm:flex"
            iconClass="bg-amber-100 text-amber-700"
          />
          <FloatingCard
            icon={MapPin}
            title="Pan-India Access"
            className="left-[-25px] sm:left-[-20px] bottom-[10%] hidden sm:flex"
            iconClass="bg-emerald-50 text-emerald-600"
          />
          <FloatingCard
            icon={Zap}
            title="Instant Routing"
            subtitle="Confirmed in Seconds"
            className="right-[-25px] sm:right-[-20px] bottom-[6%] hidden sm:flex"
            iconClass="bg-purple-100 text-[#50008C]"
          />
        </motion.div>
      </div>
